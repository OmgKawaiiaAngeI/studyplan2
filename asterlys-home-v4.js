...
wait(()=>{
 ...
 if(hero){const k=hero.querySelector('.dash-kicker'),h=hero.querySelector('h2'),p=hero.querySelector('#dashMessage');if(k)k.textContent='My study planner';if(h)h.textContent='Hello! What are we studying today?';if(p)p.textContent='Your tasks, exams, study tools and notes are all in one place.'}
 if(!$('astHomeStrip')){
